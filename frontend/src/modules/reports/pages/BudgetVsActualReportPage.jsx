...
import { getBudgetVsActual, getBudgets, resolveDateRange, downloadBudgetVsActualPdf, downloadBudgetVsActualCsv } from "../api/financialReportsApi";
...
export default function BudgetVsActualReportPage() {
  ...
  const handleDownload = async (type) => {
    ...
    let rf = from, rt = to;
    if (dateRange !== "CUSTOM") { const d = resolveDateRange(dateRange); rf = d.from; rt = d.to; }
    const params = { budgetId, accountingBasis, from: rf, to: rt };
      if (type === "pdf") await downloadBudgetVsActualPdf(params);
      else await downloadBudgetVsActualCsv(params);
