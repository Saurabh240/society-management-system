...
const Card = ({ title, children }) => (
  <div className="px-5 py-3 border-b border-gray-100" style={{ backgroundColor: "var(--color-primary-light)" }}> 
    {title && (
...
const inputClass = "w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2  disabled:bg-gray-50 disabled:opacity-60 bg-white transition";
...
const OwnershipAccountForm = ({ initialData = {}, onSubmit, loading, mode = "create" }) => {
  ...
  useEffect(() => {
    ...
    if (associations.length === 0 || allUnits.length === 0) return; // wait for dropdowns

    // If IDs already present use them directly
    let associationId = String(initialData.associationId || "");
    let unitId = String(initialData.unitId || "");

    // If IDs missing, resolve from associationName + unitNumber
    if (!associationId && initialData.associationName) {
      const matchedAssoc = associations.find(
        (a) => a.name === initialData.associationName
      );
      associationId = String(matchedAssoc?.id || "");
    }
    if (!unitId && initialData.unitNumber && associationId) {
      const matchedUnit = allUnits.find(
        (u) =>
          u.unitNumber === initialData.unitNumber &&
          String(u.associationId) === associationId
      );
      unitId = String(matchedUnit?.id || "");
    }
  ...
  return (
    <form onSubmit={handleSubmit} autoComplete="off">
      ...
      <div className="flex items-center gap-3 pt-2 pb-6">
        <button type="submit" disabled={loading} className="text-white px-6 py-2 rounded-lg text-sm font-medium disabled:opacity-50 transition hover:opacity-90"
