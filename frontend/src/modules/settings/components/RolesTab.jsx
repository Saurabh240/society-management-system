...
// ── Menu items matching the Figma screenshot ──────────────────────────────────
const MENUS = [
...
// Build default full-permission matrix
const buildDefaultPermissions = (canView = true, canEdit = true, canDelete = true) =>
  MENUS.map((m) => ({ menuKey: m.key, canView, canEdit, canDelete }));

// Map backend RoleResponse (role string + permissionLabel + userCount)
// to the richer shape the accordion needs.
// Since we don't have a real per-menu permissions API yet, we derive sensible
// defaults from the role name. When BE-3 (role permissions API) is implemented,
// replace this with a real API call.
const derivePermissionsFromRole = (roleStr) => {
  switch (roleStr) {
    case "TENANT_ADMIN":
      return buildDefaultPermissions(true, true, true);
    case "MANAGER":
      return buildDefaultPermissions(true, true, false);
    case "VIEWER":
      return buildDefaultPermissions(true, false, false);
    default:
      return buildDefaultPermissions(false, false, false);
  }
};
...
const RoleAccordion = ({ role, onEditPermissions }) => {
  ...
  return (
    <div className="border border-gray-200 rounded-lg mb-3 overflow-hidden">
      ...
      >
        {/* Left: chevron + name + badges + description */}
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {role.isDefault && (
            <span className="px-2 py-0.5 text-xs bg-gray-200 text-gray-600 rounded font-medium">
              Default
            </span>
          )}
          <span className="text-sm text-gray-400 ml-1 truncate hidden md:block">
            {role.description}
          </span>
          ...
          {role.isDefault ? (
            ...
            <>
              ...
              <button
                ...
                onClick={() => toast.info("Delete role — available in the full roles API release")}
              >
                Delete
              </button>
            </>
...
const EditPermissionsModal = ({ role, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(`Permissions for "${role.displayName}" updated`);
      onClose();
    } catch {
...
const RolesTab = () => {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      // UserController /users/roles returns ApiResponse<List<RoleResponse>>
      // RoleResponse: { role: string, permissionLabel: string, userCount: number }
      const raw = res.data.data || [];

      // Map to the shape our accordion needs
      const mapped = raw.map((r) => {
        const isAdmin = r.role === "TENANT_ADMIN";
        return {
          id:          r.role,                               // use role string as ID for now
          role:        r.role,
  ...
  const handleSavePermissions = async (roleId, permissions) => {
    // When BE-3 (role permissions API) is available, call:
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={() => toast.info("Custom role creation — available in the full roles API release")}
      ...
      {loading ? (
        ...
        roles.map((role) => (
          <RoleAccordion
            key={role.id}
            role={role}
            onEditPermissions={(r) => setEditingRole(r)}
          />
        ))
      )}
