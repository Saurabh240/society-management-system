...
import DeleteConfirmModal from "../components/DeleteConfirmModal"; // reuse existing

// ─── Constants ────────────────────────────────────────────────────────────────
const ACCOUNT_TYPES = ["All Types", "Assets", "Liabilities", "Equity", "Income", "Expenses"];
...
export default function ChartOfAccountsPage() {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      const totalElements = res.data?.totalElements ?? content.length;
      setAccounts(content);
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-3">
        ...
        <div className="flex-1">
          <label className="block text-xs text-gray-500 mb-1">Search Accounts</label>
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by name, code, or notes..."
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400 transition"
          />
        </div>
        ...
        <div>
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none transition"
          >
            {ACCOUNT_TYPES.map((t) => (
        ...
        <table className="w-full table-auto border-collapse">
          <thead style={{ backgroundColor: "#a9c3f7" }}>
            <tr>
              ...
              <th className="p-4 text-xs font-bold uppercase text-gray-800 text-center w-14" />
            </tr>
          </thead>
              ...
              accounts.map((account) => (
                <tr key={account.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4">
                    <DotsMenu
                      onEdit={() =>
                        navigate(`/dashboard/accounting/chart-of-accounts/edit/${account.id}`)
                      }
