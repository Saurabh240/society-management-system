      ...
      <div className="fixed inset-0 z-[10000] flex items-center justify-center px-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col" style={{ maxHeight: "90vh" }}>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            {/* Scheduled Date + Time — shown for create, resend, reschedule */}
            {showSchedule && (
              <>
                <div>
                  <label className={labelCls}>Scheduled Date</label>
                  <input type="date" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} placeholder="Select date" className={inputCls} />
                </div>
                <div>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200">
            ...
            {mode === "reschedule" ? (
              <button onClick={onClose} className="px-4 py-2 text-sm text-white rounded transition hover:opacity-90" style={{ backgroundColor: "var(--color-primary)" }}>
                Reschedule Email
              </button>
            ) : mode === "edit" ? (
              <>
                <button onClick={() => { onSave?.({ ...email, subject, message }); onClose(); }} className="px-4 py-2 text-sm text-white rounded transition hover:opacity-90" style={{ backgroundColor: "var(--color-primary)" }}>
                  Save Email
                </button>
                <button onClick={onClose} className="px-4 py-2 text-sm text-white rounded transition hover:opacity-90" style={{ backgroundColor: "var(--color-primary)" }}>
                  Send Email
