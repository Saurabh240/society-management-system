  ...
  return (
    ...
    associationId={associationId}
      ...
      {deleteEmailData && (
        <DeleteConfirmModal
          title="Delete Email"
          message={`Are you sure you want to delete "${deleteEmailData.subject}"?`}
          onClose={() => setDeleteEmailData(null)}
          onConfirm={() => {
            deleteEmailApi(deleteEmailData.id).then(fetchEmails);
      ...
      {showBulkDeleteConfirm && (
        <DeleteConfirmModal
          title="Batch Delete"
          message={`Are you sure you want to delete ${selected.length} emails? This action cannot be undone.`}
  ...
  return (
    <div>
      ...
      {selected.length > 0 && (
        <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-2 mb-4">
          <span className="text-sm text-gray-600">{selected.length} item{selected.length > 1 ? "s" : ""} selected</span>
          <button onClick={() => setShowBulkDeleteConfirm(true)}  className="px-3 py-1.5 text-sm text-white rounded-lg transition hover:opacity-90" style={{ backgroundColor: "var(--color-danger)" }}>
            Delete Selected
          </button>
        </div>
      )}
