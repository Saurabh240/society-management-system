...
function TextActionMenu({ item, onDeleted }) {
        ...
        <>
          ...
          <div className="fixed inset-0 z-[10000] flex items-center justify-center px-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
              <h3 className="text-base font-semibold text-gray-900 mb-2">Delete Text Message</h3>
...
export default function TextMessagePage() {
  const [messages, setMessages] = useState(TEXT_MESSAGES_DATA);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {messages.length === 0 ? (
              ...
              messages.map((item, idx) => (
                ...
                >
                  <td className="px-4 py-3 font-medium text-gray-900 underline cursor-pointer max-w-xs" style={{ borderRight: "1px solid #c8c7c7", color: "var(--color-primary)" }} onClick={() => setViewItem(item)}>{item.message}</td>
                  <td className="px-4 py-3 text-gray-700 whitespace-nowrap" style={{ borderRight: "1px solid #c8c7c7" }}>{item.recipient}</td>
                  <td className="px-4 py-3 text-gray-700" style={{ borderRight: "1px solid #c8c7c7" }}>{item.phone}</td>
                  <td className="px-4 py-3 text-gray-700 whitespace-nowrap" style={{ borderRight: "1px solid #c8c7c7" }}>{item.date}</td>
                  <td className="px-4 py-3" style={{ borderRight: "1px solid #c8c7c7" }}><StatusBadge status={item.status} /></td>
                  <TextActionMenu item={item} onDeleted={(id) => setMessages((p) => p.filter((m) => m.id !== id))} />
                </tr>
              ))
            )}
          </tbody>
